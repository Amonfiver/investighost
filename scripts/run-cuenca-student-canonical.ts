...
const STUDENT = {
  title: 'Cuenca (España): una ciudad entre paisaje, historia, arte y ciencia',
  content: `## [intro] Introducción
Cuenca, en España, se comprende a partir de una relación especialmente estrecha entre territorio y ciudad. Las hoces de los ríos Júcar y Huécar, la historia de su casco, los monumentos, el arte abstracto, la paleontología y las tradiciones forman un conjunto de temas conectados. En Cuenca, geografía, historia, cultura y ciencia no aparecen como capítulos aislados: forman una misma trama cultural.
...
## [heritage] Patrimonio y arquitectura
...
## [nature_science] Naturaleza, geología y ciencia
...
## [daily_life] Sociedad y tradiciones
...
Ajoarriero, morteruelo, pisto y resolí son referencias de la gastronomía tradicional de Cuenca. La cocina, igual que las fiestas o la artesanía, participa en la forma en que una comunidad reconoce y transmite elementos de identidad local.

## [budget] Datos y conceptos clave
...
## [practical] Relaciones para comprender Cuenca
...
El relieve aporta el marco geográfico; 1177 ofrece un eje histórico; los monumentos traducen esa historia en patrimonio; el museo vincula arquitectura y arte; Las Hoyas une territorio y paleontología; y las tradiciones incorporan una dimensión social. Geografía, historia, patrimonio, arte y ciencia se explican mutuamente. El casco histórico depende de un paisaje concreto, mientras que la paleontología amplía esa lectura hacia un pasado mucho más antiguo. Cultura y gastronomía completan una identidad local que incluye prácticas colectivas, además de edificios, fechas y colecciones.

Esta perspectiva permite situar en un mismo horizonte las hoces, las Casas Colgadas, el museo, la Serranía y Las Hoyas. Cada elemento pertenece a un ámbito distinto, pero todos contribuyen a una ciudad cuya identidad se ha formado entre paisaje, memoria, creación cultural y conocimiento científico.

## [risks] Visión de conjunto
...
const elegance = assertStudentElegance({ ...STUDENT, evidence: EVIDENCE, sourceLimited: false })
...
console.log(JSON.stringify({
  operation: 'cuenca_student_elegant_071',
  adventureUnchanged: true,
  gates: { publicSafe: 'PASS', contentTooThin: 'NO', readability: 'PASS', scannability: 'PASS', studentDepth: 'PASS', studentEducationalValue: 'PASS', studentNonTravel: 'PASS', studentInformationalValue: 'PASS', studentEducationalStructure: 'PASS', studentEncyclopedicTone: 'PASS', studentNonSchoolish: 'PASS', studentSectionElegance: 'PASS', studentNoVisibleNumberedSyllabus: 'PASS', studentNoMetaeducationalLanguage: 'PASS', profileDifferentiation: 'PASS' },
  student: { wordCount: depth.wordCount, sections: 12, topicsDeveloped: depth.developedThemes, thematicCoverage: canonical.thematicCoverage, namedPlaceCoverage: canonical.namedPlaceCoverage, datesUsed: canonical.datesUsed, questionsIncluded: canonical.questionsIncluded, paragraphCount: readability.paragraphCount, longParagraphCount: readability.longParagraphCount, travelLanguage: canonical.travelLanguage, schoolishLanguage: elegance.schoolishLanguage, rawInternalMarkersVisible: internalMarkers(student.content), markdownEscapedVisible: /\\(?:\*|_|`|\[|\]|#)/u.test(student.content) },
...
async function createStudentVersion(): Promise<string> {
  ...
  const result = await drafts.createDraft({ libraryEntryId: ENTRIES.student, expectedHeadHash, title: STUDENT.title, content: STUDENT.content, changeSummary: 'student_elegant_071: convierte Student en una enciclopedia editorial, no turística y no escolar basada en investigación durable.', actorId: MANUAL_LOCAL_ACTOR_ID, operationKey: key('investighost:071:cuenca:student:elegant:v1') })
...
async function approveStudentVersion(versionId: string): Promise<void> {
  ...
  const state = await versioning.getVersionStateSnapshot(versionId)
  const submitted = await decisions.submitForReview({ versionId, revisionId: detail.currentRevision.id, expectedState: 'draft', expectedRevisionHash: detail.currentRevision.revisionHash, expectedTraceabilityHash: state.traceabilityHash, reason: 'student_elegant_071: perfil enciclopédico, elegante, no turístico y no escolar.', actorId: MANUAL_LOCAL_ACTOR_ID, operationKey: key('investighost:071:cuenca:student:submit:v1') })
  ...
  const decision = await decisions.decideVersion({ versionId, revisionId: detail.currentRevision.id, decisionType: 'approve', expectedPreviousState: 'ready_for_review', expectedDecisionTargetHash: review.aggregateHash, reason: 'student_elegant_071 aprobada: explica Cuenca como conocimiento enciclopédico, sin planificación de viaje ni tono escolar.', actorId: MANUAL_LOCAL_ACTOR_ID, affectedFindingKeys: [], changeInstructions: [], acceptedRiskFindingKeys, separationOfDutiesException: true, separationOfDutiesReason: 'Operador local autorizado aprueba Student tras validar tono enciclopédico, elegancia editorial, seguridad pública y ausencia de turismo o ficha escolar.', operationKey: key('investighost:071:cuenca:student:approve:v1') })
