-- The shared versioning transaction historically accepted only pilot entries
-- already transferred to the Library. Batch entries are deliberately created
-- before human approval, so they use the same version/revision transaction
-- with a narrower candidate-state guard.
...
begin
  ...
  if entry.origin = 'real_editorial_batch_job' then
    if entry.status <> 'candidate'
       ...
       or entry.transfer_id is not null
       or entry.pilot_id is not null
       or entry.run_id is not null
       or entry.execution_owner_id is null then
  ...
  if found then
    ...
    head_hash := (select d.decision_target_hash from public.real_editorial_library_version_decisions d
      where d.version_id = previous.id and d.decision_type in ('approve','request_changes','reject','abandon')
  ...
  version_hash := public.real_editorial_library_hash(jsonb_build_object(
    ...
    'parentHash',head_hash,'createdBy',actor_id,'createdAt',public.real_editorial_library_timestamp(created_at),
  ...
  revision_hash := public.real_editorial_library_hash(jsonb_build_object(
    ...
    'revisionNumber',1,'previousRevisionId',null,'previousRevisionHash',null,'contentHash',content_hash,
    'createdBy',actor_id,'createdAt',public.real_editorial_library_timestamp(created_at),
  ...
  insert into public.real_editorial_library_versions (
    id,library_entry_id,version_number,parent_version_id,parent_origin_version_hash,parent_hash,version_hash,
    canonicalization_contract,creation_reason,created_by_actor_id,created_at,operation_key,request_fingerprint,publication_state
  ) values (
  ...
  return jsonb_build_object(
    'status','ok','operation','create_version','reused',false,'versionId',version_id,
    'revisionId',revision_id,'state','draft','versionHash',version_hash,'revisionHash',revision_hash,
    'traceabilityHash',null,'decisionTargetHash',null
  );
end $$;

revoke all on function public.real_editorial_library_create_version(jsonb) from public, anon, authenticated;
